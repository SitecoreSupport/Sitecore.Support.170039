    ...
    public class LayoutDetailsForm : DialogForm
    {
        ...
        protected void CopyDevicePipeline(ClientPipelineArgs args)
        {
            ...
            else
            {
                ...
                WebUtil.SetSessionValue(GetSessionKey(), doc.OuterXml);
                SheerResponse.ShowModalDialog(new UrlString(UIUtil.GetUri("control:CopyDeviceTo"))
                {
                    ["de"] = StringUtil.GetString(new string[] { args.Parameters["deviceid"] }),
        ...
        {
            return "SC_DEVICEEDITOR_" + GetCurrentItem().ID.Guid.ToString();
        }
        ...
        protected void EditRenderingPipeline(ClientPipelineArgs args)
        {
            ...
            if (parameters.Show())
            {
                XmlDocument document2 = XmlUtil.LoadXml(WebUtil.GetSessionString(GetSessionKey()));
                WebUtil.SetSessionValue(GetSessionKey(), null);
        ...
        protected void OpenDevicePipeline(ClientPipelineArgs args)
        {
            ...
            if (args.IsPostBack)
            {
                if (!string.IsNullOrEmpty(args.Result) && (args.Result != "undefined"))
                {
                    XmlDocument doc = XmlUtil.LoadXml(WebUtil.GetSessionString(GetSessionKey()));
                    WebUtil.SetSessionValue(GetSessionKey(), null);
                    if (doc != null)
                    {
            ...
            else
            {
                ...
                WebUtil.SetSessionValue(GetSessionKey(), document2.OuterXml);
        ...
        public virtual string FinalLayout
        {
            get
            {
                ...
                if (!string.IsNullOrWhiteSpace(this.Layout) && XmlPatchUtils.IsXmlPatch(layoutDelta))
                {
        ...
        public virtual string Layout
        {
            ...
            {
                return
         StringUtil.GetString(base.ServerProperties["Layout"]);
            }
            set
            {
                Assert.ArgumentNotNull(value, "value");
        ...
        protected virtual string LayoutDelta
        {
            ...
            {
                return
         ((string)base.ServerProperties["LayoutDelta"]);
            }
            set
            {
        ...
        {
            get { return MainUtil.GetBool(base.ServerProperties["VersionCreated"], false); }
